...
DIR_BASE = '/home/omewangyx/kaggle/carvical_cancer/zhouyusong_code/data/train_data_path'
use_gpu = torch.cuda.is_available()
BATCH_SIZE = 32
kfold = 7

model_name_base = 'models/resnet50_ft_k_{}'
...
def exp_lr_scheduler(optimizer, epoch, init_lr=0.001, lr_decay_epoch=16, min_lr=.99e-8):
...
#    model_ft.classifier._modules['6'] = nn.Linear(4096, 3)
    ...
    for param in list(model_ft.parameters())[:-32]:
         param.requires_grad = False

    if use_gpu:
        model_ft = model_ft.cuda()
    criterion = nn.CrossEntropyLoss(size_average=True)
    optimizer_ft = optim.SGD(list(model_ft.parameters())[-32:], lr=0.01, momentum=0.9)
...
for i in range(kfold):
    ...
    print('-'*10, 'Start training model {}/{}'.format(i, kfold), '-'*10)
    start_train(kid=i, n_epoch=48)
